...
class SnipeITServer(models.Model):
    ...
    def IOS_run(self, serialnumber, devicename, jss_url, jss_user, jss_password):
        ...
        snipeit_ios_response = requests.get(snipe_url, headers=snipe_headers)
        snipeit_ios_data = snipeit_ios_response.json()
        if snipeit_ios_response.status_code != 200:
            return "NON 200 STATUS"
        else:
            """If Device Not In Snipe"""
            if snipeit_ios_data["total"] == 0:
                ...
                try:
                    jss_asset_response = requests.get(
                        "{}/JSSResource/mobildevices/serialnumber/{}".format(
                            jss_url, serialnumber
                        ),
                        auth=(jss_user, jss_password),
                        headers=jss_headers,
                    ...
                    if jss_asset_response.status_code != 200:
                        ...
                        jss_asset_result = json.loads(jss_asset_response.text)
                        # print(jss_asset_response.text)
                        jss_ios_name = jss_asset_result["mobile_device"]["general"][
                            "name"
                        ]
                        jss_asset_tag = jss_asset_result["mobile_device"]["general"][
                            "asset_tag"
                        ]
                        jss_user_name = jss_asset_result["mobile_device"]["location"][
                            "username"
                        ]
                        jss_real_name = jss_asset_result["mobile_device"]["location"][
                            "real_name"
                        ]
                        # jss_email = (jss_asset_result['computer']['location']['email_address'])
                        # jss_phone = (jss_asset_result['computer']['location']['phone'])
                        ...
                        # jss_building = (jss_asset_result['computer']['location']['building'])
                        # jss_purchase_date = (jss_asset_result['computer']['purchasing']['po_date'])
                        jss_model = jss_asset_result["mobile_device"]["model"]
                        if jss_asset_tag == "":
                            return "No asset Tag for {} in Jamf, Cannot Create in Snipe".format(
                                jss_ios_name
                            )
                        else:
                            """This is Where the device gets created in snipe if possible"""
                            ...
                            if snipeit_model_response.status_code != 200:
                                ...
                                if snipeit_model_data["rows"][0]["name"] != jss_model:
                                    ...
                                    snipe_create_asset["model_id"] = snipe_model_id
                                    snipe_create_asset["name"] = jss_ios_name
                                    snipe_create_asset["serial"] = serialnumber
                                    # return("Device",jss_computer_name, jss_asset_tag, jss_model, "Not in Snipe")
                                    snipe_create_asset_url = (
                                        self.url + "/api/v1/hardware/"
